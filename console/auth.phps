...
$console = new Console();
	if(isset($_GET['code'])){
		...
		if(isset($token['access_token'])){
			...
			$q = $console->engine->query_result("SELECT * FROM `console_users` WHERE uid = '".$userInfo['id']."'");
			$hash = md5($console->generate_hash());
			if(isset($q->uid)){
				$console->engine->query("UPDATE `console_users` SET hash = '".$hash."' WHERE uid = '".$userInfo['id']."'");
				$_SESSION['phpmc_id'] = $q->id;
				$_SESSION['phpmc_uid'] = $q->uid;
				$_SESSION['phpmc_hash'] = $hash;
			}else{
				$console->engine->query("INSERT INTO `console_users`(`first_name`, `last_name`, `hash`, `uid`) VALUES ('{$userInfo['first_name']}', '{$userInfo['last_name']}', '{$hash}', '{$userInfo['id']}')");
