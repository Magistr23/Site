...
function mcraftQuery_SE($host, $port = 25565, $timeout = 3)
{
...
function mcraftQuery($host, $port = 25565, $timeout = 1)
{
    ...
    } // if ask server to much it return empty data

    $t = explode("\x00",
        $tmp[0]);                          // Divide the first part from every NULL-terminated string end into key1 val1 key2 val2...
...
function mcraftPing($host, $port = 25565, $timeout = 3)
{

    $query = new \xPaw\MinecraftPing($host, $port, $timeout);
    try {
        $data = $query->Query();
    ...
    return [
        'hostname'   => $host,
        'numplayers' => isset($data['players']['max']) ? (int) $data['players']['max'] : 0,
        'maxplayers' => isset($data['players']['online']) ? (int) $data['players']['online'] : 0,
    ];
}
